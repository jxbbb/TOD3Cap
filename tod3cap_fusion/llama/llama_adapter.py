...
class LLaMA_adapter(nn.Module):
    ...
    def forward(self, cap_inputs, det_inputs):
        ...
        feats, obj_pred_box = det_inputs #torch.Size([4, 32400, 256]) torch.Size([4, 9])

        bbox_query = self.forward_visual(det_inputs) #torch.Size([4, 10, 4096])

        _bsz, seqlen = tokens.shape #4 512
